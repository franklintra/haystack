...
struct imgfs_header {
    char name[MAX_IMGFS_NAME + 1]; // +1 for null terminator
    uint32_t version;
    uint32_t nb_files;
    uint32_t max_files;
    uint16_t resized_res[2 * (NB_RES - 1)]; // Array for resized resolutions
    uint32_t unused_32;
    uint64_t unused_64;
};

struct img_metadata {
    char img_id[MAX_IMG_ID + 1]; // +1 for null terminator
    unsigned char SHA[SHA256_DIGEST_LENGTH];
    uint32_t orig_res[2]; // Resolution of original image (width, height)
    uint32_t size[NB_RES]; // mem sizes for thumbnail, small, and original res
    uint64_t offset[NB_RES]; // pos in the img db file for each res
    uint16_t is_valid; // whether the img is in use (NON_EMPTY) or not (EMPTY)
    uint16_t unused_16;
};

struct imgfs_file {
    FILE* file; // pointer to the file containing the img db
    struct imgfs_header header; // info about the img db
    struct img_metadata* metadata; // dynamic array of metadata for img in the db
};

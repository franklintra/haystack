...
START_TEST(http_match_uri_additional_tests)
{
    start_test_print;

    struct http_message http_msg;
    http_msg.uri.val = "/universal/resource/identifier";
    http_msg.uri.len = strlen(http_msg.uri.val);
...
// ======================================================================
START_TEST(http_match_verb_additional_tests)
{
    start_test_print;

    struct http_string http_str;
    http_str.val = "POST / HTTP/1.1";
    http_str.len = 4;

    ck_assert_int_eq(http_match_verb(&http_str, "POST"), 1);

    http_str.val = "GET / HTTP/1.1";
    http_str.len = 3;

    ck_assert_int_eq(http_match_verb(&http_str, "GET"), 1);
    ck_assert_int_eq(http_match_verb(&http_str, "GET /"), 0);
    ck_assert_int_eq(http_match_verb(&http_str, "G"), 0);
...
Suite *http_test_suite()
{
    ...
    Add_Test(s, http_match_uri_additional_tests);


    Add_Test(s, http_match_verb_null_params);
    Add_Test(s, http_match_verb_valid);
    Add_Test(s, http_match_verb_additional_tests);
